...
1. **`event.rs`** — `TicketEvent`/`ReviewEvent` enums (`TicketCreated`, `StatusChanged`, `Assigned`, `TicketCommented` / `ReviewOpened`, `CommentAdded`, `VerdictSet`), each serializable to exactly one JSON line (`to_line`/`from_line`).
